...
class PickerLocationHeight extends Ui.Picker {
  ...
  function initialize() {
    ...
    if($.SA_Settings.iUnitElevation == Sys.UNIT_STATUTE) {
    ...
    Picker.initialize({
      :title => new Ui.Text({ :text => Lang.format("$1$ [$2$]", [Ui.loadResource(Rez.Strings.titleLocationHeight), $.SA_Settings.sUnitElevation]), :font => Gfx.FONT_TINY, :locX=>Ui.LAYOUT_HALIGN_CENTER, :locY=>Ui.LAYOUT_VALIGN_BOTTOM, :color => Gfx.COLOR_BLUE }),
      :pattern => [ new PickerFactoryNumber(0, $.SA_Settings.iUnitElevation == Sys.UNIT_STATUTE ? 29 : 9, null), oText_10e3,
